...
void recursiveCopying(std::shared_ptr<TBinaryTreeItem> parItem, std::shared_ptr<TBinaryTreeItem> curItem,
                      std::shared_ptr<TBinaryTreeItem> otherItem, bool isLeftChild){
    if (otherItem != nullptr){
        curItem = std::make_shared<TBinaryTreeItem>(TBinaryTreeItem(otherItem));
        if (isLeftChild){
...
TBinaryTree::TBinaryTree(TBinaryTree& otherBinTree){
    this->root = std::make_shared<TBinaryTreeItem>(TBinaryTreeItem(otherBinTree.root));
...
void TBinaryTree::Push(const Octagon& oct){
    if (this->root == nullptr){
        this->root = std::make_shared<TBinaryTreeItem>(TBinaryTreeItem(oct));
    }
    else if (this->root->GetOctagon() == oct){
        ++this->root->counter;
    }
    else{
        std::shared_ptr<TBinaryTreeItem> parent = this->root;
        std::shared_ptr<TBinaryTreeItem> curItem;
        ...
        curItem = std::make_shared<TBinaryTreeItem>(TBinaryTreeItem(oct));
...
void TBinaryTree::Pop(const Octagon& oct){
    std::shared_ptr<TBinaryTreeItem> deletedItem = root;
    if (root != nullptr) { // if tree isn't empty
        std::shared_ptr<TBinaryTreeItem> parentDelItem = root;
        ...
        if (deletedItem->counter > 1){
            ...
            if (deletedItem->GetLeft() != nullptr){ // check left subtree
                std::shared_ptr<TBinaryTreeItem> largestChild = deletedItem->GetLeft();
                if (largestChild->GetRight() != nullptr) { // if he isn't the largest child himself
                    std::shared_ptr<TBinaryTreeItem> parent = largestChild;
                ...
                std::shared_ptr<TBinaryTreeItem> leastChild = deletedItem->GetRight();
                if (leastChild->GetLeft() != nullptr) { // if he isn't the least child himself
                    std::shared_ptr<TBinaryTreeItem> parent = leastChild;
...
void recursiveCount(const Octagon& octagon, std::shared_ptr<TBinaryTreeItem> curItem, int& ans){
    if (curItem != nullptr){
...
void recursiveClear(std::shared_ptr<TBinaryTreeItem> curItem){
    if (curItem != nullptr){
...
Octagon& TBinaryTree::GetItemNotLess(double area) {
    std::shared_ptr<TBinaryTreeItem> curItem = root;
...
void recursivePrint(std::shared_ptr<TBinaryTreeItem> curItem, int& closeBracketAmount, std::string& resStr){
    if (curItem != nullptr){
...
std::shared_ptr<TBinaryTreeItem> TBinaryTree::GetRoot(){
    return root;
}
